...
public interface TvRepository extends JpaRepository<TvEntity, Long> {
    ...
    @Query(value = "select * from tv inner join person on tv.person_id = person.id inner join tv_category on tv.tv_category_external = tv_category.id where person.email = ?1 and tv.date_of_correction is null order by tv.reserved_date_to_repair desc", nativeQuery = true)
    List<TvEntity> findAllNotRepairedTvByPersonEmail(String personEmail);
